...
describe("dashboard card modernization", () => {
  it("does not use inline glass imitations (bare Card with bg-card/50 backdrop-blur)", () => {
    ...
    expect(markupOf(<QueueMixCard pendingTotal={4} queueSegments={queueSegments} />)).not.toContain(
      "bg-card/50"
    );
  ...
  it("QueueMix pending badge does not use 1:1 text-primary on bg-primary/10", () => {
    expect(markupOf(<QueueMixCard pendingTotal={4} queueSegments={queueSegments} />)).not.toMatch(
      /bg-primary\/10[^"]*text-primary|text-primary[^"]*bg-primary\/10/
    );
  });
});
