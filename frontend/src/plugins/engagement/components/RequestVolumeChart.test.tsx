...
describe("RequestVolumeChart", () => {
  it("renders an empty state when no requests exist", () => {
    render(<RequestVolumeChart rows={[row()]} />);
    expect(screen.getByText("No requests yet")).toBeInTheDocument();
  });

  it("renders the chart instead of the empty state when there is volume", () => {
    const withVolume = row({
      metrics: {
        leave: { ...emptyType(), submitted: 4, decided: 3, approved: 2, rejected: 1 },
        overtime: emptyType(),
        standby: emptyType(),
      },
    });
    render(<RequestVolumeChart rows={[withVolume]} />);
